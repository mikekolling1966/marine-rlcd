...
void PCF85063_Init()
{
	if (!I2C_DevicePresent(PCF85063_ADDRESS)) {
		s_rtc_available = false;
		printf("PCF85063 not detected at 0x%02X\r\n", PCF85063_ADDRESS);
		I2C_Scan();
		return;
	}

	uint8_t Value = RTC_CTRL_1_DEFAULT|RTC_CTRL_1_CAP_SEL;
	esp_err_t write_ret = I2C_Write(PCF85063_ADDRESS, RTC_CTRL_1_ADDR, &Value, 1);
	esp_err_t read_ret = I2C_Read(PCF85063_ADDRESS, RTC_CTRL_1_ADDR,  &Value, 1);
	if (write_ret != ESP_OK || read_ret != ESP_OK) {
		s_rtc_available = false;
		printf("PCF85063 failed to be initialized. write=%d read=%d\r\n", write_ret, read_ret);
	}
	else if(Value & RTC_CTRL_1_STOP) {
		s_rtc_available = false;
...
{
	if (!s_rtc_available || time == nullptr) return;
	uint8_t buf[7] = {0};
