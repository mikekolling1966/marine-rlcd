...
/* NOTE: The original implementation used IDF I2C master APIs (i2c_master_write_to_device etc.).
 * In some build configurations those symbols may be unavailable at link time. To avoid
 * link errors, the IDF implementation is disabled here and an alternative C++ implementation
 * using the Arduino Wire library is provided in `esp_io_expander_tca9554.cpp`.
 */
