...
void LCD::Config::convertPartialToFull()
{
    ...
    if (std::holds_alternative<DevicePartialConfig>(device)) {
        ...
        device = DeviceFullConfig{
            ...
            /*
             * Prior implementation defaulted to BIG-endian unconditionally which
             * causes unwanted byte-swapping for typical little-endian RGB565 data
             * buffers (LVGL/Arduino). Use LITTLE as the safe default unless the
             * user explicitly requests otherwise via driver-level configuration.
             */
...
void LCD::Config::printDeviceConfig() const
{
    ...
    if (std::holds_alternative<DeviceFullConfig>(device)) {
        ...
        ESP_UTILS_LOGI(
            ...
            , static_cast<int>(config.data_endian)
